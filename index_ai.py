...
original_points = points.copy()  # Store original to avoid cumulative rotation
...
while running:
    ...
    # Rotate a copy of the original points each frame
    scaled_points = scale(original_points, theta1)
    rotated_points = rotate_x(scaled_points, theta1*100)

    # Vectorized screen coordinate calculation
    screen_x = rotated_points[:, 0] + rotated_points[:, 2] * k[0]
    screen_y = rotated_points[:, 1] + rotated_points[:, 2] * k[1]

    # Create mask for valid coordinates
    valid_mask = (screen_x >= 0) & (screen_x < screen_width) & (screen_y >= 0) & (screen_y < screen_height)

    # Get only valid points
    valid_x = screen_x[valid_mask].astype(np.int32)
    valid_y = screen_y[valid_mask].astype(np.int32)

    # Direct pixel writing using surfarray
    if len(valid_x) > 0:
        pixel_array = pygame.surfarray.pixels3d(screen)
        for i in range(len(valid_x)):
            pixel_array[valid_x[i], valid_y[i]] = white
        del pixel_array
    ...
    maximum_vertices_per_fps = 50000
