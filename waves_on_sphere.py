...
while running:
    ...
    for i in range(200):
        my_theta += math.pi/30
        my_theta_horiz += math.pi/10
        for i in range(200):
            my_theta2 += math.pi/30
            my_theta_horiz2 += math.pi/50
            x = 200+100*math.sin(my_theta)*math.cos(my_theta2)     +5*math.cos(my_theta2+theta*10)   #<- cool wave effect
            y = (100)*math.sin(my_theta)*math.sin(my_theta2)     +5*math.sin(my_theta2+theta*20)   #<- cool wave effect
            z = -100+100*math.cos(my_theta) 
        #    x2 = 300+100*math.sin(my_theta_horiz)*math.cos(my_theta_horiz2)*math.sin(theta)
        #    y2 = 100*math.sin(my_theta_horiz)*math.sin(my_theta_horiz2)
        #    z2 = -300+100*math.cos(my_theta_horiz)*math.cos(theta)
            draw_point(x,y,z, (x+y)*0.005+1)
        #    draw_point(x2,y2,z2)
    ...
    for i in range(40):
        ...
        for i2 in range(20):
            y2 = (5*math.sin(i*math.pi/30 + (i2*math.pi/30+theta*20)))
            draw_point(270+((i*5)), -300+((i2*10)), -150+y2)
...
messy variant of rotating cube
